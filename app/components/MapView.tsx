...
const VARIANT_SOURCES: Record<string, { label: string; url: string; type: string }[]> = {
  ...
  THREATS: [
    { label: 'Reuters Health', url: 'https://www.reuters.com/news/health', type: 'Global Wire' },
    { label: 'AP News Health', url: 'https://apnews.com/health', type: 'Global Wire' },
    { label: 'BBC Health', url: 'https://www.bbc.com/news/health', type: 'Global Media' },
    { label: 'WHO Newsroom', url: 'https://www.who.int/news-room', type: 'Official' },
    { label: 'CNN Health', url: 'https://edition.cnn.com/health', type: 'Global Media' },
...
// Major international outlets that signal a truly global event
const AUTHORITY_OUTLETS = [
  'reuters', 'associated press', 'ap news', 'bbc', 'cnn', 'who', 'afp',
  'bloomberg', 'al jazeera', 'new york times', 'washington post',
  'wall street journal', 'guardian', 'sky news', 'xinhua', 'time magazine',
];

const CRITICAL_KEYWORDS = [
  'pandemic', 'epidemic', 'global outbreak', 'public health emergency', 'pheic',
  'who declares', 'mass casualty', 'natural disaster', 'earthquake', 'tsunami',
  'hurricane', 'catastrophe', 'death toll', 'lockdown', 'bioterror', 'nuclear emergency',
];

// Only triggers if content references 2+ authority outlets AND a critical keyword
function isCriticalThreat(content: string): boolean {
  const lower = content.toLowerCase();
  const hasKeyword = CRITICAL_KEYWORDS.some(kw => lower.includes(kw));
  const outletCount = AUTHORITY_OUTLETS.filter(o => lower.includes(o)).length;
  return hasKeyword && outletCount >= 2;
}

...
export default function MapView({ activeVariants, region, isDark = true }: {
  ...
  const modalBorder = isDark ? '55' : '33';

  useEffect(() => {
    if (!activeTopic) { setActiveSignals({}); return; }
    if (!activeSignals[activeTopic]) {
      setActiveSignals(prev => ({...prev,[activeTopic]:[...(TOPIC_SIGNALS[activeTopic]??[])]}));
    }
  }, [activeTopic]);
      ...
      .then(results => {
        ...
        results.forEach((result,i) => {
          (result.items??[]).forEach((item:{headline:string;source:string;time:string;link:string}) => {
            ...
            let age = 720;
            if (m) { const n=parseInt(m[1]),u=m[2].toLowerCase()[0]; age=u==='m'?n:u==='h'?n*60:n*1440; }
  ...
  useEffect(() => {
    ...
    const map=L.map(mapRef.current,{center:[25,20],zoom:2,zoomControl:false,attributionControl:false,worldCopyJump:false,maxBounds:[[-85,-180],[85,180]],maxBoundsViscosity:1.0});
  ...
  useEffect(() => {
    ...
    if (mapMode!=='live') {
      staticLocs.forEach(loc => {
        const topicActive=activeVariants.length
cat > ~/Desktop/global-health-watch/app/components/MapView.tsx << 'ENDOFFILE'
'use client';
import { useEffect, useRef, useState, useCallback } from 'react';
  ...
  useEffect(() => {
    ...
    if (mapMode!=='live') {
      staticLocs.forEach(loc => {
        ...
        const icon=(L as any).divIcon({
          className:'',
          html:`<div style="width:10px;height:10px;border-radius:50%;background:${c}22;border:2px solid ${c};box-shadow:0 0 5px ${c}44;cursor:pointer;"></div>`,
    ...
    if (mapMode!=='directory') {
      liveDots.forEach(dot => {
        ...
        const icon=(L as any).divIcon({
          className:'',
          html:`<div style="width:${size}px;height:${size}px;border-radius:50%;background:${c};opacity:${0.55+freshness*0.45};border:1px solid ${c}cc;box-shadow:0 0 ${isPulse?12:5}px ${c}${isPulse?'cc':'66'};cursor:pointer;${isPulse?'animation:mapPulse 2s ease-in-out infinite;':''}"></div>`,
          iconSize:[size,size],iconAnchor:[size/2,size/2],
        });
        const marker=L.marker([dot.lat,dot.lng],{icon}).addTo(map)
          .on('click',()=>setModal({title:dot.headline,subtitle:`${dot.source} · ${dot.time}`,link:dot.link,topic:dot.topic,isLive:true}));
        markersRef.current.push(marker);
        lCount++;
      });
    }
  ...
  return (
    <div style={{position:'relative',width:'100%',backgroundColor:'#0a0a0f'}}>
      ...
      {activeTopic&&signals.length>0&&(
        <div style={{position:'absolute',top:'12px',left:'12px',zIndex:1000,backgroundColor:bg,border:`1px solid ${c(activeTopic)}44`,borderRadius:'10px',minWidth:'170px',overflow:'hidden',boxShadow:`0 8px 32px rgba(0,0,0,0.3)`}}>
          <div onClick={()=>setSignalsPanelOpen(p=>!p)} style={{padding:'8px 12px',display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',borderBottom:signalsPanelOpen?`1px solid ${c(activeTopic)}33`:'none'}}>
          ...
          {signalsPanelOpen&&(
            <div style={{padding:'6px 0'}}>
              {signals.map(signal=>{
                ...
                return (
                  <div key={signal} onClick={()=>toggleSignal(activeTopic,signal)}
      ...
      <div style={{position:'absolute',top:'12px',left:'50%',transform:'translateX(-50%)',zIndex:1000,display:'flex',gap:'6px',alignItems:'center'}}>
        ...
        <div style={{display:'flex',gap:'2px',backgroundColor:bg,border:`1px solid ${border}`,borderRadius:'8px',padding:'3px'}}>
          {([['both','Both'],['live','● Live'],['directory','○ Dir']] as const).map(([mode,label])=>(
            <button key={mode} onClick={()=>setMapMode(mode)} style={{backgroundColor:mapMode===mode?(isDark?'rgba(255,255,255,0.1)':'rgba(0,0,0,0.08)'):'transparent',color:mapMode===mode?textPrimary:textMuted,border:'1px solid transparent',borderRadius:'5px',padding:'3px 9px',fontSize:'11px',fontWeight:600,cursor:'pointer',transition:'all 0.15s',whiteSpace:'nowrap'}}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div style={{position:'absolute',bottom:'20px',left:'12px',zIndex:1000,backgroundColor:bg,border:`1px solid ${border}`,borderRadius:'8px',padding:'7px 12px',display:'flex',gap:'12px',alignItems:'center',flexWrap:'wrap'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'5px'}}>
          <div style={{width:'9px',height:'9px',borderRadius:'50%',backgroundColor:isDark?'rgba(255,255,255,0.7)':'rgba(0,0,0,0.5)',boxShadow:'0 0 5px rgba(255,255,255,0.4)'}}/>
      ...
      {/* DRAG HANDLE */}
      <div onMouseDown={onDragStart} style={{position:'absolute',bottom:0,left:0,right:0,height:'8px',cursor:'ns-resize',display:'flex',alignItems:'center',justifyContent:'center',backgroundColor:'transparent',zIndex:1001}}>
        <div style={{width:'40px',height:'3px',borderRadius:'2px',backgroundColor:isDark?'rgba(255,255,255,0.15)':'rgba(0,0,0,0.15)'}}/>
      </div>

      {/* MODAL */}
      {modal&&(
        <div style={{position:'fixed',inset:0,zIndex:2000,backgroundColor:'rgba(0,0,0,0.55)',display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}} onClick={()=>setModal(null)}>
          <div style={{backgroundColor:modalBg,border:`1px solid ${c(modal.topic)}${modalBorder}`,borderRadius:'14px',maxWidth:'420px',width:'100%',padding:'24px',boxShadow:`0 24px 80px rgba(0,0,0,0.4),0 0 40px ${c(modal.topic)}22`}} onClick={e=>e.stopPropagation()}>
            {/* Topic + signal tag */}
            ...
            {/* Title */}
            <div style={{fontSize:'15px',fontWeight:700,color:textPrimary,lineHeight:1.45,marginBottom:'6px'}}>{modal.title}</div>
            {/* Description for directory, source+time for live */}
            {modal.description&&<div style={{fontSize:'12px',color:c(modal.topic),marginBottom:'4px',fontWeight:500}}>{modal.description}</div>}
            <div style={{fontSize:'11px',color:textMuted,marginBottom:'20px'}}>{modal.subtitle}</div>
            {/* Actions */}
            <div style={{display:'flex',gap:'10px',alignItems:'center'}}>
              ...
              <button onClick={()=>setModal(null)} style={{background:'none',border:`1px solid ${border}`,borderRadius:'8px',padding:'9px 16px',color:textMuted,fontSize:'12px',cursor:'pointer'}}>
                Close
              </button>
            </div>
          </div>
      ...
      <style>{`@keyframes mapPulse{0%,100%{transform:scale(1);opacity:1;}50%{transform:scale(1.5);opacity:0.6;}}`}</style>
    </div>
  );
}
